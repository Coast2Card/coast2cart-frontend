...
const CartConfirmationModal = ({ isOpen, onClose, product, seller }) => {
  ...
  return (
    <div className="fixed inset-0 flex justify-end items-start pt-20 pr-10 z-50" onClick={onClose}>
      <div ref={modalRef} className="bg-white rounded-xl p-6 w-[450px] max-w-[90vw] shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center mb-5">
          ...
          <div className="flex items-center gap-2">
            <span className="text-base font-medium text-gray-800">{seller?.name || "Juan Dela Cruz"}</span>
            <span className="bg-blue-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold">✓</span>
          </div>
        </div>
        ...
        <div className="flex gap-4 mb-5">
          ...
          <div className="flex-1">
            <h3 className="text-lg font-bold text-gray-800 mb-2">{product?.name || "Bangus"}</h3>
            <p className="text-sm text-gray-600 mb-2 leading-relaxed">
              {product?.description || "Fresh Chilled Milkfish All Sizes"}
            </p>
            <p className="text-base font-bold text-gray-800">P{product?.price || 289}</p>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center mb-5">
          <div className="flex items-center border border-gray-300 rounded-md overflow-hidden">
            ...
            <span className="px-4 py-2 border-l border-r border-gray-300 text-base font-medium min-w-[40px] text-center">{quantity}</span>
            <button 
              className="bg-white border-none px-3 py-2 cursor-pointer text-base font-bold text-gray-800 hover:bg-gray-50 transition-colors" 
              onClick={() => handleQuantityChange(1)}
            >
          ...
          <button className="bg-none border-none text-orange-500 text-sm cursor-pointer underline p-0 hover:text-orange-600" onClick={handleRemove}>
            Remove
          </button>
        </div>
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <span className="text-lg font-bold text-green-600">P{totalPrice.toFixed(2)}</span>
        </div>

        {/* Action Buttons */}
        <div className="flex gap-3">
          <button className="flex-1 bg-yellow-400 text-gray-800 border-none px-6 py-3 rounded-lg text-base font-semibold cursor-pointer hover:bg-yellow-500 transition-colors" onClick={handleViewCart}>
            View Cart
          </button>
          <button className="flex-1 bg-gray-100 text-gray-800 border border-gray-300 px-6 py-3 rounded-lg text-base font-medium cursor-pointer hover:bg-gray-200 transition-colors" onClick={onClose}>
            Continue Shopping
          </button>
        </div>
      </div>
