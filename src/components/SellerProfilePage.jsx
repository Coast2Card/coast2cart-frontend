...
import { MapPin, Phone, Envelope, Calendar, MagnifyingGlass, Star } from '@phosphor-icons/react';
...
const SellerProfilePage = ({ sellerId }) => {
  ...
  const reviews = [
    ...
    },
    {
      ...
      rating: 5,
      comment: "Best seafood in Baybayon! Always fresh and reasonably priced. Highly recommended!",
      date: "1 week ago",
      avatar: customer2
    },
    {
  ...
  const SellerProfileHeader = () => (
    <div className="bg-gradient-to-r from-primary via-[#0058BA] to-primary rounded-lg p-6 mb-6 relative overflow-hidden">
      <div className="flex flex-col md:flex-row items-center gap-6 relative z-10">
        ...
        <div className="flex-1 text-center md:text-left text-white">
          <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
            <h1 className="text-3xl font-bold font-display">{sellerData.name}</h1>
          ...
          <div className="space-y-2 text-white/90">
            ...
            <div className="flex items-center justify-center md:justify-start gap-2">
              <Calendar size={18} />
              <span>Seller Since {sellerData.memberSince}</span>
            </div>
          </div>
        </div>

        <div className="hidden md:block">
          <img src={c2cLogo} alt="Coast2Cart" className="h-16 opacity-80" />
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="dropdown dropdown-end">
            <label tabIndex={0} className="btn btn-outline btn-sm font-primary hover:scale-105 hover:border-primary/50 hover:bg-blue-50 transition-all duration-200 active:scale-95">
              {selectedCategory === 'all' ? 'All Categories' :
               selectedCategory === 'fresh_catch' ? 'Fresh Catch' :
               selectedCategory === 'dried_seafood' ? 'Dried Seafood' : 'Souvenirs'}
              <svg className="w-4 h-4 ml-1 transition-transform duration-200 group-hover:rotate-180" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
              </svg>
            </label>
            <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow-lg bg-white rounded-xl w-52 border border-gray-100 animate-fade-scale-in">
              <li><a onClick={() => setSelectedCategory('all')} className="font-primary hover:bg-blue-50 rounded-lg transition-colors duration-200">All Categories</a></li>
              <li><a onClick={() => setSelectedCategory('fresh_catch')} className="font-primary hover:bg-blue-50 rounded-lg transition-colors duration-200">Fresh Catch</a></li>
              <li><a onClick={() => setSelectedCategory('dried_seafood')} className="font-primary hover:bg-blue-50 rounded-lg transition-colors duration-200">Dried Seafood</a></li>
              <li><a onClick={() => setSelectedCategory('souvenirs')} className="font-primary hover:bg-blue-50 rounded-lg transition-colors duration-200">Souvenirs</a></li>
            </ul>
          </div>
          ...
          <div className="relative">
            <input
              ...
              spellCheck="false"
            />
            <MagnifyingGlass size={16} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none" />
            {searchTerm && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
  ...
  const ProductCard = ({ product, isSold = false }) => (
    ...
    >
      ...
      <div className="p-4 text-center group-hover:bg-blue-50/50 transition-all duration-200">
        {!isSold ? (
          <>
            <div className="text-success text-xl font-bold mb-1 font-primary group-hover:text-primary transition-all duration-200 group-hover:scale-105">{product.price}</div>
            <div className="text-gray-800 font-semibold text-lg mb-1 font-primary group-hover:text-primary transition-colors duration-200">{product.name}</div>
            <div className="text-gray-500 text-sm font-primary group-hover:text-gray-600">{product.availability}</div>
          </>
        ) : (
          <>
            <div className="text-success text-2xl font-bold mb-1 font-primary group-hover:scale-105 transition-transform duration-200">{product.quantity}</div>
            <div className="text-gray-800 font-semibold text-lg mb-1 font-primary">{product.name}</div>
            <div className="text-amber-600 text-sm font-primary font-medium">{product.soldDate}</div>
          </>
        )}
      </div>
    </div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-semibold text-gray-800 font-primary group-hover:text-primary transition-colors duration-200 truncate">{review.customerName}</h3>
            <span className="text-gray-400 text-sm font-primary flex-shrink-0 ml-2">{review.date}</span>
          </div>
          <div className="flex items-center mb-3 gap-1">
            {[...Array(5)].map((_, i) => (
              <Star
                ...
                size={16}
                weight="regular"
                className={`transition-all duration-200 hover:scale-110 ${i < review.rating ? 'text-secondary hover:text-orange-500' : 'text-gray-300 hover:text-gray-400'}`}
              />
            ))}
            <span className="ml-2 text-sm text-gray-500 font-primary">({review.rating}/5)</span>
          </div>
          <p className="text-gray-600 leading-relaxed font-primary group-hover:text-gray-700 transition-colors duration-200">{review.comment}</p>
        </div>
      </div>
    </div>
  );
    ...
    <div>
      <div className="bg-white rounded-lg p-4 mb-6 shadow-sm border">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="dropdown dropdown-end">
              <label tabIndex={0} className="btn btn-outline btn-sm font-primary">
                {selectedCategory === 'all' ? 'All Categories' :
                 selectedCategory === 'fresh_catch' ? 'Fresh Catch' :
                 selectedCategory === 'dried_seafood' ? 'Dried Seafood' : 'Souvenirs'}
              ...
              <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                <li><a onClick={() => setSelectedCategory('all')} className="font-primary">All Categories</a></li>
                <li><a onClick={() => setSelectedCategory('fresh_catch')} className="font-primary">Fresh Catch</a></li>
                <li><a onClick={() => setSelectedCategory('dried_seafood')} className="font-primary">Dried Seafood</a></li>
                <li><a onClick={() => setSelectedCategory('souvenirs')} className="font-primary">Souvenirs</a></li>
              </ul>
            </div>
